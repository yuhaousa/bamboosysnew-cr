<template>
  <div class="space-y-4">
    <FieldGroup label="Badge">
      <input v-model="local.badge" @input="emit" class="form-input" placeholder="Optional badge" />
    </FieldGroup>
...
import type { CenteredTextContent } from '@shared/types'

const props = defineProps<{ content: CenteredTextContent }>()
const emit_ = defineEmits<{ update: [CenteredTextContent] }>()
const local = reactive<CenteredTextContent>({ ...props.content })
