<template>
  <div class="p-6 space-y-6">
    <div class="flex items-center justify-between">
      ...
      <button @click="openForm()" class="btn-primary text-sm">+ Add FAQ</button>
    </div>

    <!-- Modal Form -->
    <AppModal v-model="showForm" :title="editing?.id ? 'Edit FAQ' : 'New FAQ'" size="xl">
      <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="md:col-span-2"><label class="form-label">Question *</label><input v-model="form.question" class="form-input" /></div>
        <div class="md:col-span-2"><label class="form-label">Answer *</label><textarea v-model="form.answer" class="form-textarea" rows="5" /></div>
        <div><label class="form-label">Category</label><input v-model="form.category" class="form-input" placeholder="e.g. Pricing" /></div>
      ...
      <template #footer>
        ...
        <button @click="save" :disabled="saving" class="btn-primary text-sm">{{ saving ? 'Saving…' : 'Save FAQ' }}</button>
      </template>
    </AppModal>
    ...
    <div class="card overflow-hidden !p-0">
      ...
      <table v-else class="w-full text-sm">
        <thead class="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700">
          <tr>
            <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Question</th>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/40">
            <td class="px-4 py-3 max-w-xs">
              <p class="font-medium text-gray-900 dark:text-white truncate">{{ item.question }}</p>
              <p class="text-xs text-gray-400 truncate">{{ item.answer }}</p>
            </td>
            <td class="px-4 py-3 text-gray-500 dark:text-gray-400 hidden md:table-cell">{{ item.category || '—' }}</td>
            <td class="px-4 py-3">
          ...
          <tr v-if="!items.length"><td colspan="4" class="px-4 py-8 text-center text-gray-400">No FAQs yet.</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
function blank() {
  return { question: '', answer: '', category: '', isActive: true, sortOrder: 0 }
}

onMounted(async () => {
  try { const d = await api.get<{ data: any[] }>('/faqs?limit=200'); items.value = d.data ?? [] }
...
async function save() {
  if (!form.value.question || !form.value.answer) return toast.error('Question and answer are required')
  saving.value = true
  try {
    if (editing.value?.id) {
      const d = await api.put<{ data: any }>(`/faqs/${editing.value.id}`, toApi(form.value))
      const idx = items.value.findIndex(i => i.id === editing.value.id)
      if (idx >= 0) items.value[idx] = d.data
    } else {
      const d = await api.post<{ data: any }>('/faqs', toApi(form.value))
