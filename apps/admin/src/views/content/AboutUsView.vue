<template>
  <div class="p-6 space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">About Us</h1>
        <p class="text-sm text-gray-500 mt-0.5">Manage about us sections and their detail rows</p>
      </div>
      <button @click="openCreate()" class="btn-primary text-sm">+ Add About Us</button>
    </div>

    <AppModal v-model="showCreateForm" title="New About Us" size="lg">
      <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="flex items-center gap-2 pt-7">
          <input id="about-us-active" v-model="createForm.isActive" type="checkbox" class="rounded" />
          <label for="about-us-active" class="text-sm text-gray-700 dark:text-gray-300">Active</label>
        </div>
      </div>
      <template #footer>
        <button @click="showCreateForm = false" class="btn-secondary text-sm">Cancel</button>
        <button @click="saveCreate" :disabled="savingCreate" class="btn-primary text-sm">{{ savingCreate ? 'Saving…' : 'Save About Us' }}</button>
      </template>
    </AppModal>

    <AppModal v-model="showEditForm" :title="editForm.name ? `Edit ${editForm.name}` : 'Edit About Us'" size="2xl">
      <div class="p-6 space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="flex items-center gap-2 pt-7">
            <input id="edit-about-us-active" v-model="editForm.isActive" type="checkbox" class="rounded" />
            <label for="edit-about-us-active" class="text-sm text-gray-700 dark:text-gray-300">Active</label>
          </div>
        </div>
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-4">
          <div class="flex items-center justify-between">
            <div>
              <h2 class="text-sm font-semibold text-gray-900 dark:text-white">About Us Rows</h2>
              <p class="text-xs text-gray-500">Add multiple title, image, and description rows for this section.</p>
            </div>
          ...
          <div v-if="!editForm.entries.length" class="rounded-xl border border-dashed border-gray-300 dark:border-gray-700 p-6 text-center text-sm text-gray-400">
            No rows yet. Add one to start building this section.
          </div>
          ...
          <div v-for="(entry, index) in editForm.entries" :key="entry.id || index" class="rounded-xl border border-gray-200 dark:border-gray-700 p-4 space-y-4">
            ...
            <div>
              ...
              <ImagePicker v-model="entry.imageUrl" folder="about-us" />
            </div>
      ...
      <template #footer>
        ...
        <button @click="saveEdit" :disabled="savingEdit" class="btn-primary text-sm">{{ savingEdit ? 'Saving…' : 'Save About Us' }}</button>
      </template>
    </AppModal>
    ...
    <div class="card overflow-hidden !p-0">
      ...
      <table v-else class="w-full text-sm">
        <thead class="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700">
          <tr>
            <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">About Us</th>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/40">
            ...
            <td class="px-4 py-3 text-right flex justify-end gap-2">
              ...
              <button @click="removeItem(item.id)" class="text-xs text-red-400 hover:underline">Delete</button>
            </td>
          </tr>
          <tr v-if="!items.length">
            <td colspan="4" class="px-4 py-8 text-center text-gray-400">No about us sections yet.</td>
          </tr>
        </tbody>
      </table>
    </div>
...
function blankEdit() {
  return { id: '', name: '', isActive: true, sortOrder: 0, entries: [] as AboutUsEntry[] }
}

async function load() {
  loading.value = true
  try {
    const res = await api.get<{ data: AboutUsItem[] }>('/about-us?limit=200')
    items.value = (res.data || []).map((item) => ({ ...item, entries: item.entries || [] }))
  } catch (e: any) {
    toast.error(e.message || 'Failed to load about us')
  } finally {
    loading.value = false
  }
}
...
async function saveCreate() {
  if (!createForm.value.name.trim()) return toast.error('About us name is required')
  savingCreate.value = true
  try {
    const res = await api.post<{ data: AboutUsItem }>('/about-us', {
    ...
    toast.error(e.message || 'Failed to create about us')
  } finally {
...
async function openEdit(id: string) {
  try {
    const res = await api.get<{ data: AboutUsItem }>(`/about-us/${id}`)
    editForm.value = {
      id: res.data.id,
      name: res.data.name,
...
async function saveEdit() {
  if (!editForm.value.name.trim()) return toast.error('About us name is required')
  savingEdit.value = true
  try {
    const res = await api.put<{ data: AboutUsItem }>(`/about-us/${editForm.value.id}`, {
    ...
    toast.error(e.message || 'Failed to save about us')
  } finally {
...
async function removeItem(id: string) {
  if (!confirm('Delete this about us section?')) return
  try {
    await api.delete(`/about-us/${id}`)
    items.value = items.value.filter((item) => item.id !== id)
    toast.success('Deleted')
  } catch (e: any) {
    toast.error(e.message || 'Failed to delete about us')
  }
}
