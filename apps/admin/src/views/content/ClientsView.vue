  ...
  <div class="p-6 space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">Manage your clients</p>
      </div>
      <button @click="openForm()" class="btn-primary text-sm">+ Add Client</button>
    </div>

    <AppModal v-model="showForm" :title="editing?.id ? 'Edit Client' : 'New Client'" size="xl">
      <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div><label class="form-label">Client Name *</label><input v-model="form.name" class="form-input" /></div>
        ...
        <div class="md:col-span-2">
          ...
          <ImagePicker v-model="form.imageUrl" folder="clients" />
        </div>
      ...
      <template #footer>
        ...
        <button @click="save" :disabled="saving" class="btn-primary text-sm">{{ saving ? 'Saving…' : 'Save Client' }}</button>
      </template>
    </AppModal>
    ...
    <div class="card overflow-hidden !p-0">
      ...
      <table v-else class="w-full text-sm">
        <thead class="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700">
          <tr>
            <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Client</th>
            <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400 hidden md:table-cell">Icon</th>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/40">
            ...
            <td class="px-4 py-3 text-gray-500 dark:text-gray-400 hidden md:table-cell">{{ item.icon || '-' }}</td>
            <td class="px-4 py-3">
          ...
          <tr v-if="!items.length"><td colspan="4" class="px-4 py-8 text-center text-gray-400">No clients yet.</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
function blank() {
  return { name: '', imageUrl: '', icon: '', isActive: true, sortOrder: 0 }
}

onMounted(async () => {
  try { const d = await api.get<{ data: any[] }>('/clients?limit=200'); items.value = d.data ?? [] }
...
async function save() {
  if (!form.value.name) return toast.error('Client name is required')
  saving.value = true
  try {
    if (editing.value?.id) {
      const d = await api.put<{ data: any }>(`/clients/${editing.value.id}`, toApi(form.value))
      const idx = items.value.findIndex(i => i.id === editing.value.id)
      if (idx >= 0) items.value[idx] = d.data
    } else {
      const d = await api.post<{ data: any }>('/clients', toApi(form.value))
