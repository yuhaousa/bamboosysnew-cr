<template>
  <section :style="sectionStyle" class="section-padding relative overflow-hidden">
    ...
    <div class="relative container-content text-center max-w-2xl mx-auto space-y-6 z-10">
      ...
      <h2 data-cms-field="title" class="heading-lg" :class="content.backgroundImage?.url ? 'text-white' : 'text-gray-900 dark:text-white'">{{ content.title }}</h2>
      <p v-if="content.description" data-cms-field="description" class="text-lg" :class="content.backgroundImage?.url ? 'text-gray-200' : 'text-gray-600 dark:text-gray-400'">{{ content.description }}</p>
